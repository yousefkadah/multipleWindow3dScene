class WindowManager 
{
	...
	init (metaData)
	{
		// Make sure #windows is always initialized as an array
		try {
			const storedWindows = localStorage.getItem("windows");
			this.#windows = storedWindows ? JSON.parse(storedWindows) : [];
			// Double check that it's an array
			if (!Array.isArray(this.#windows)) {
				console.warn("Stored windows was not an array, resetting to empty array");
				this.#windows = [];
			}
		} catch (e) {
			console.error("Error parsing windows from localStorage:", e);
			this.#windows = [];
		}
